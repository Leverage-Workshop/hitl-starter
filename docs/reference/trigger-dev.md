...
> Moved out of `CLAUDE.md` to keep the root harness file short. This is generic
> Trigger.dev vendor reference — the installed `trigger-*` skills (see
> `.claude/skills/`) are the primary, always-current source. Note: `@trigger.dev/sdk`
> is **not yet a dependency** of this project; these blocks are forward-looking guidance
> for when external engines are wired in over webhooks (see `docs/API.md`).
>
> The `<!-- TRIGGER.DEV ... -->` markers below delimit managed blocks; leave them intact.
