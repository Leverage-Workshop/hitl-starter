/* ============================================================
   Config page — definition-list layout (NOT cards).
   Configures the active workflow. Includes an action-config
   section that drives the flyout's decision buttons.
   ============================================================ */

const ConfigPage = ({ workflowId, onNavigate }) => {
  const wf = WORKFLOWS.find((w) => w.id === workflowId) || WORKFLOWS[0];

  const [name, setName] = useState(wf.name);
  const [trigger, setTrigger] = useState('email · rfps@halberd-co.com');
  const [schedule, setSchedule] = useState('on-receipt · max 5/min');
  const [confFloor, setConfFloor] = useState(80);
  const [actionSet, setActionSet] = useState(ACTION_SET.map((a) => a.label).join(' · '));
  ...
  return (
    <>
      <Topbar
        ...
        right={<>
          <Button onClick={() => onNavigate('dashboard')}>Cancel</Button>
      ...
      <div className="page" data-screen-label="03 Config">
        ...
        <dl className="deflist">
          ...
          <dd>
            ...
            <div className="desc" style={{ marginTop: 8 }}>Routed via: <span className="mono">{trigger}</span></div>
          </dd>
          ...
          <dd>
            <div className="desc">Below this confidence, items are routed to a human. Above it, items can auto-send if "auto-decide" is on.</div>
            <input
              type="text"
              value={confFloor}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxWidth: 480 }}>
              {ACTION_SET.map((a, i) => (
                <div key={a.key} style={{
                  ...
                  <span style={{
                    color:
                      a.variant === 'brass'  ? 'var(--fg-accent)' :
                      a.variant === 'danger' ? 'var(--c-oxide)' :
                      'var(--fg-muted)',
