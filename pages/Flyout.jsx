/* ============================================================
   DetailFlyout — slides in from the right.

   Body is a configurable slot. By default it renders a two-pane
   compare (source vs AI draft) plus a marginalia notes column.
   For workflows without a comparison axis, swap the body
   renderer at the marked extension point.

   The action bar is ALSO a configurable slot — driven by
   ACTION_SET in data.js.
   ============================================================ */

const DetailFlyout = ({ item, onClose, onAction }) => {
  const open = !!item;
        ...
        {item && (
          <>
            ...
            {/* body — CONFIGURABLE SLOT.
                ...
                For workflows without an AI draft, replace with a single-pane
                renderer (e.g. just the source + notes). */}
            <div className="flyout__body">
              <FlyoutTwoPane item={item} />
            </div>

            {/* action bar — CONFIGURABLE SLOT.
                Buttons come from ACTION_SET in data.js. Swap that array to
...
const FlyoutTwoPane = ({ item }) => (
  <div className="flypanes">
    {/* LEFT: source — what came in */}
    <div className="flypane">
      <div className="flypane__label">
        <span>SOURCE · INBOUND</span>
        <span style={{ color: 'var(--fg-muted)' }}>{item.attachments ? `${item.attachments} ATCH` : 'NO ATCH'}</span>
      </div>
      <div className="flypane__field">
        <div className="flypane__field-label">FROM</div>
        <div className="flypane__field-value mono" style={{ fontFamily: 'var(--font-mono)', fontSize: 13 }}>{item.from}</div>
      </div>
...
Object.assign(window, { DetailFlyout, FlyoutTwoPane });
